...
function getDb() {
  if (!db) {
    ...
    // Ensure data directory exists, owner-only. The chmod is unconditional:
    // mode on mkdir only applies when creating, so existing installs would
    ...
    // The message store is private to this user, not readable by other accounts.
    for (const suffix of ['', '-wal', '-shm']) {
      try { fs.chmodSync(dbPath + suffix, 0o600); } catch { /* may not exist yet */ }
    }
