...
export async function analyzeMessage({
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      const parsed = robustJsonParse(content, { allowPartial: attempt === MAX_ATTEMPTS });
  ...
  if (/fetch failed|ECONNREFUSED|Failed to fetch|not reachable|terminated|ECONNRESET|socket hang up|other side closed/i.test(m)) {
