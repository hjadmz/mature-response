...
// "Nothing leaves this computer" is the promise. If OLLAMA_URL points
// anywhere but loopback, that promise no longer holds — say so once, loudly.
try {
  const host = new URL(OLLAMA).hostname;
  if (!['localhost', '127.0.0.1', '::1', '[::1]'].includes(host)) {
    console.warn(
      `[mature-response] OLLAMA_URL points to a non-local host (${host}). ` +
      'Your messages will be sent to that machine for analysis. ' +
      'Unset OLLAMA_URL to keep everything on this computer.'
    );
  }
} catch { /* unparseable URL fails at fetch time with a clear error */ }
