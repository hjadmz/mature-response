...
export function robustJsonParse(content, { allowPartial = true } = {}) {
  let jsonStr = content;
  ...
  try {
    ...
    try {
      ...
      try {
        // Synthesizing an analysis from defaults is a last resort: on any
        // attempt but the final one, fail so the retry can get real output.
        if (!allowPartial) throw new Error('UNPARSEABLE_MODEL_RESPONSE');
        const extract = (field) => {
