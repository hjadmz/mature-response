...
// The received message is untrusted text written by someone else. Fencing it
// keeps injected newlines and forged section headers inside the fence, where
// they cannot occupy a structural position the model is told to trust.
const FENCE_OPEN = '<<<MESSAGE';
const FENCE_CLOSE = 'MESSAGE>>>';
const strip = (s) => String(s || '').replaceAll(FENCE_OPEN, '').replaceAll(FENCE_CLOSE, '');
...
export function buildUserPrompt({ mode, message_text, context_type, perceived_tone, emotional_reaction, feeling, optional_note, desired_outcome, outcomePriors }) {
  ...
  const noteLine = optional_note ? `\nAdditional context: ${strip(optional_note)}` : '';
...
What I want to say / the situation:
${FENCE_OPEN}
${strip(message_text)}
${FENCE_CLOSE}
