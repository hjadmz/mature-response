...
export function enforceDecisionRules(parsed, { mode, emotional_reaction, feeling, needClarify }) {
  ...
  if (parsed.risk_level === 'high' && parsed.intent_guess === 'provocation') {
    if (mode === 'communication') {
      // The user is the one sending. "Ignore" would delete the draft they
      // asked for, so hold it instead of discarding it.
      parsed.engagement_level = 'wait';
      if (!parsed.reason || !parsed.reason.trim()) {
        parsed.reason = 'This reads as provocation. Sit with it before sending anything.';
      }
    } else {
