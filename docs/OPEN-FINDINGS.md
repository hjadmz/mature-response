...
From a twelve-agent audit (six reviewers, each finding independently verified
by an adversarial checker). Fixed items are removed as they land — this file
is the remaining work, not the history. Read each fix before implementing:
several of them correct a naive first attempt.

Last updated 2026-08-19.

## 1. [medium] Cross-origin CSRF writes: any web page can silently POST to /api/outcome and /api/analyze

**File:** `mature-response/app/api/outcome/route.js`
...
## 2. [medium] Database of private messages is created world-readable (0644)

**File:** `mature-response/lib/db.js`
...
## 3. [medium] Received message is interpolated into the prompt inside bare unescaped quotes, letting a hostile sender forge prompt sections

**File:** `mature-response/lib/ai/prompts.js`
...
## 4. [medium] README overstates the high-emotion rule: intensity alone never forces a pause

**File:** `mature-response/README.md`
...
## 5. [medium] Export-all and erase-all are undocumented and absent from the screenshots
...
## 6. [medium] Node >=20.9.0 is enforced in package.json but stated nowhere, and the launchers only check that node exists

**File:** `mature-response/start.command`
...
## 7. [medium] The sovereignty hardening pass is committed locally but never pushed, so the public repo is the un-hardened version
...
## 8. [medium] All three routes share one document.title, so Next's route announcer never fires — client navigation is silent

**File:** `mature-response/app/layout.js`
...
## 9. [medium] No headings below h1 anywhere in the app — visual section titles are divs, so nothing is navigable

**File:** `mature-response/components/InsightsPanel.js`
...
## 10. [medium] Delete and erase-all are confirmed but never announced, and focus is dumped to <body> afterwards

**File:** `mature-response/app/history/page.js`
...
## 11. [medium] Delete, erase-all, and save-outcome fail silently — res.ok is never checked and errors go only to the console
...
## 12. [medium] A failed read is shown as "you have no data" — History says "No Analyses Yet", Insights renders blank stat cards
...
## 13. [medium] Write failures are silently swallowed — "Save Outcome", delete and erase-all can no-op with zero feedback

**File:** `mature-response/components/OutcomeLogger.js`
...
## 14. [medium] Any DB error is reported as "Analysis failed. Try again in a moment." and a completed analysis is thrown away

**File:** `mature-response/app/api/analyze/route.js`
...
## 15. [medium] History silently shows only the 100 most recent entries, so the "Needs Outcome" filter misses older pending ones
...
## 16. [medium] Debounced search captures a stale context filter, so the list can contradict the dropdown
...
## 17. [medium] Two-column .form-row silently truncates the Desired outcome select between 601px and ~718px

**File:** `mature-response/app/globals.css`
...
## 18. [medium] Nine interactive controls are under the 44px touch minimum, including the destructive Erase all history button
...
## 19. [medium] Fixed 10-11px labels never scale and sit far below any readability floor
...
## 20. [low] /api/outcome skips the input-clamping discipline the analyze route documents as mandatory
...
## 21. [low] RISK_LEVELS is exported but never imported; RiskBadge re-declares the same labels inline

**File:** `mature-response/lib/constants.js`
...
## 22. [low] SETUP.md quotes an out-of-memory message the app never displays

**File:** `mature-response/SETUP.md`
...
## 23. [low] Debounced search can resolve after a filter change and repaint the previous filter's results
...
## 24. [low] Header claims "AAA contrast" but the caption token is AA (4.55:1), and it fails on the raised surface
...
## 25. [low] Unguarded response shape, and a connection dropped mid-response is not classified — both surface as the generic "try again" message

**File:** `mature-response/lib/ai/ollama.js`
...
## 26. [low] A failed export navigates the app away to a raw JSON error page
...
## 27. [low] Mode toggle spills outside its own border at 280px
...
## 28. [low] Vertical rhythm doubles where .form-row collapses to one column
